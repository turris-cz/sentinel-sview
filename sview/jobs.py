...
from .data_helpers import get_and_store, process_query, DBTask, DBMultiTask
...
def load_data_from_template(resource_name):
    task = DBTask(resource_name).from_dict(
    dbtask = DBTask.from_dict(resource_name, QUERIES[resource_name])
    dbtask.process()
    dbtask.store()
