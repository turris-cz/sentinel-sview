...
__all__ = ["Fwlogs", "MinipotFtp", "MinipotHttp", "MinipotSmtp", "MinipotTelnet"]
...
class MinipotFtp(BaseModel):
    action = CharField()  # ('connect', 'login', 'invalid')
    asn = IntegerField(null=True)
    country = CharField(null=True)
    id = UnknownField(null=True, primary_key=True)  # BIGSERIAL
    identity = ForeignKeyField(
...
class MinipotHttp(BaseModel):
    action = CharField()  # ('connect', 'login', 'invalid', 'message')
    asn = IntegerField(null=True)
    country = CharField(null=True)
    id = UnknownField(null=True, primary_key=True)  # BIGSERIAL
    identity = ForeignKeyField(
...
class MinipotSmtp(BaseModel):
    ...
    ip = UnknownField()  # INET
    mechanism = CharField(null=True)  # ('login', 'plain')
    password = TextField(null=True)
    ts = BigIntegerField()
    username = TextField(null=True)
